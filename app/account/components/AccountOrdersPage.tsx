...
export default function AccountOrdersPage() {
  ...
  // modal state for clearing all orders
  ...
  async function handleClearConfirm() {
    ...
    setClearError(null);
    try {
      await clearPastOrders(user.uid);
      setIsClearModalOpen(false);
    } catch (err) {
      console.error("Failed to clear orders:", err);
      setClearError("Failed to clear orders. Please try again.");
      // keep modal open so user can retry/cancel
    } finally {
      setIsClearing(false);
    }
  }
  ...
  return (
    <div className="w-full">
      {/* Processing card */}
      <OrderProcessingCard visible={isProcessingVisible} onClose={() => setIsProcessingVisible(false)} />

      {/* Confirm clear modal */}
      <ConfirmClearModal
        open={isClearModalOpen}
        title="Clear all past orders"
      ...
      >
        <div className="space-y-2">
          <p>This will permanently remove all past orders from your account.</p>
      ...
      <div
        className={`${isProcessingVisible ? "opacity-60 pointer-events-none select-none" : ""} 
                       transition-opacity flex flex-col gap-1 lg:gap-3 xl:gap-4`}
      >
...
export default function AccountOrdersPage() {
  ...
  return (
    <div className="w-full">
      ...
      <div className={`${isProcessingVisible ? "opacity-60 pointer-events-none select-none" : ""} 
                       transition-opacity flex flex-col gap-1 lg:gap-3 xl:gap-4`}>
        <div className="flex items-center mb-3 gap-3">
          <FiShoppingBag className="h-6 w-6 text-orange-800" aria-hidden="true" />
        ...
        {orders.length > 0 ? (
          <>
            <section className="flex flex-col gap-4 my-4">
              {orders.map((order) => (
                <article key={order.id} className={`flex gap-4 px-4 py-4 md:py-6 bg-white items-center
                  ...
                  <div className={`flex-shrink-0 w-16 h-16 rounded-md bg-orange-50
                                   flex items-center justify-center text-orange-800 font-medium
                                   overflow-hidden`}>
                      <div className="flex flex-col items-center justify-center gap-1">
                        <FaUtensils className="h-5 w-5" aria-hidden="true" />
                      </div>
            ...
            <div className="mt-4 md:mx-auto md:w-10/12 xl:w-8/12">
              <Button 
                onClick={() => clearPastOrders(user.uid)} 
                className="w-full inline-flex items-center justify-center gap-2 "
              >
                <FiXCircle className="h-4 w-4" aria-hidden="true" />
                Clear all
