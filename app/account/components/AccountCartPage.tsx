...
export default function AccountCartPage() {
  ...
  async function handleConfirmOrder() {
    if (!user || isPlacing) return;
    setOrderError(null);
    setIsPlacing(true);

    // Client-side quick check using profile credit (fast UX)
    const currentCredit = Number(profile?.jadeLilyCredit ?? 0);
    if (currentCredit < total) {
      setOrderError("You don’t have enough credit to complete this order.");
      setIsPlacing(false);
      return;
    }
    ...
    try {
      ...
      // assume server clears cart on success and profile updates
      setConfirmOpen(false);
      router.push("/account/orders?processing=1");
    } catch (err: unknown) {
      console.error("Place order failed", err);
      // Server-side check (defensive)
  ...
  return (
    <div className="w-full flex flex-col gap-1 lg:gap-3 xl:gap-4">
      ...
      >
        ...
        <div className="space-y-3">
          <p className="text-sm text-stone-600">
            You&apos;re about to place an order with {cart.length} item{cart.length !== 1 ? "s" : ""}.
          </p>

          <ul className="max-h-40 overflow-auto divide-y divide-stone-100">
            {cart.map((it) => (
              <li key={it.cartItemId} className="py-2 flex items-center gap-3">
                <div className={`w-10 h-10 rounded-md bg-orange-50 overflow-hidden
      ...
      {cart.length === 0 ? (
        <section aria-live="polite" className="rounded-lg p-6 grad-primary shadow-md">
        ...
        <>
          <ul className="space-y-4 mb-6 list-none" aria-live="polite">
            {cart.map((item) => (
              <li 
                key={item.cartItemId ?? `${item.dishId}-${item.name}`} 
              ...
              >
                ...
                >
                  <div className={`flex-shrink-0 w-16 h-16 rounded-md bg-orange-50 flex items-center
                                   justify-center text-orange-800 font-medium overflow-hidden`}>
          ...
          <div className={`rounded-lg p-4 md:p-6 grad-primary shadow-md flex flex-col
